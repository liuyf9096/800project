...
QString FLogServer::saveToTxtEnable(bool en, quint64 index)
{
    QString textfileName;
    Q_D(FLogServer);
    if (en == true) {
        if (d->mLogFile.isOpen() == false) {
            if (d->mLogFile.fileName().isEmpty()) {
                textfileName = d->creatLogFile(index);
            }
            ...
            if (ok) {
                ...
                qInfo() << "[Logger] Start log at path:" << d->mLogFile.fileName();
                return textfileName;
            } else {
                qWarning() << "[Logger]log txt file open Fail." << d->mLogFile.fileName();
                return QString();
            }
        } else {
            return d->mLogFile.fileName();
...
FLogServerPrivate::~FLogServerPrivate()
{
    ...
    if (mThread->isFinished()) {
        qInfo() << "Logger thread quit finished.";
    } else {
        qWarning() << "Logger thread can not quit. timeout.";
    }
}

#if 0
QString appPath = QCoreApplication::applicationDirPath();
QString otherPath = QStandardPaths::writableLocation(QStandardPaths::ApplicationsLocation);
QString otherPath = QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation);
SetFileAttributes((LPCWSTR)(filePath.unicode()), FILE_ATTRIBUTE_HIDDEN);    //#include "windows.h"
#endif
